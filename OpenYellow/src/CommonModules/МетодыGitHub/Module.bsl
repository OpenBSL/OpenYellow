...
Процедура ОбновитьДанныеРелизов() Экспорт
    ...
    ПолучитьДанныеПоРелизу(ТДС - (60 * 60 * 24 * 7));
...
Процедура ОбработатьЗаписьДанныхРепозитория(Знач Репозиторий)
	...
	Если Не ЗначениеЗаполнено(ТекущийРепозиторий.Ссылка) И Не ТекущийРепозиторий.ЭтоФорк Тогда
		
		Если СтрНайти(ТекущийРепозиторий.Описание, "Битрикс") = 0 И
			СтрНайти(ТекущийРепозиторий.Описание, "битрикс") = 0 И
			СтрНайти(ТекущийРепозиторий.Описание, "Bitrix") = 0 И
			СтрНайти(ТекущийРепозиторий.Описание, "bitrix") = 0 Тогда
    ...
    Если ВыборкаДетальныеЗаписи.Следующий() Тогда
        ...
        Если НоваяВерсия Тогда
            ...
            Если ЗначениеЗаполнено(ДатаПубликации)  Тогда
                ...
                Если ДатаПубликации > Дата(2024, 6, 30) Тогда 
