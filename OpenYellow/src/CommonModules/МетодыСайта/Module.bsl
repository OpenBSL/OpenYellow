...
Процедура ВыгрузитьИндексы() Экспорт
    ...
    КаталогИндексов = Константы.ПапкаДанных.Получить() + ИмяКаталога;
...
Процедура ОбновитьДанныеСайта() Экспорт
    ...
    Для Н = 1 По 2 Цикл
        ...
        Попытка
            ...
            ЛокальныйКаталог = Константы.ПапкаДанных.Получить();
            УдаленныйКаталог = "/data";
            Токен            = Константы.ТокенNeocities.Получить();
...
Процедура ВыгрузитьТаблицу(Знач НомерТаблицы)
    ...
    ДД.Записать(Константы.ПапкаДанных.Получить() + ДанныеТаблицы["ИмяФайла"]);
...
Процедура ВыгрузитьСчетчики()
    ...
    ОтветJSON.Закрыть();
    ДД = ПотокОтвета.ЗакрытьИПолучитьДвоичныеДанные();
    ДД.Записать(Константы.ПапкаДанных.Получить() + "counters.json");
...
Процедура ВыгрузитьАвторов()
    ...
    ДД.Записать(Константы.ПапкаДанных.Получить() + "authors.json");
...
Функция ЗаписатьДанныеЗначка(Знач Место, Знач Код, Знач Группа)
	...
	Место    	= OPI_Инструменты.ЧислоВСтроку(Место);
	Группа      = OPI_Инструменты.ЧислоВСтроку(Группа);
    ПапкаДанных	= Константы.ПапкаДанных.Получить();
