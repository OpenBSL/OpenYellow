...
Процедура ОбновитьДанныеОРепозиториях(Знач Страница = 1, Знач Запрос = "") Экспорт
	...
	Данные = ИнструментарийВызовСервера.ПолучитьСписокРепозиториев(Запрос, Страница);
...
Процедура ПолучитьДанныеПоРелизу(Знач РубежОбновления)
    ...
    Если ВыборкаДетальныеЗаписи.Следующий() Тогда
        ...
        ПоследнийРелиз = ИнструментарийВызовСервера.ПолучитьПоследнийРелиз(Репозиторий);
