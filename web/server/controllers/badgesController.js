...
exports.getBadgeSvg = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`
            ...
            FROM repos r1 WHERE id = ?
        `, [id]);

        if (rows.length === 0) {
            return res.status(404).send('<svg xmlns="http://www.w3.org/2000/svg" width="117" height="20"><text x="10" y="15" fill="red">Not Found</text></svg>');
        }

        const svg = generateBadgeSvgString(`#${rows[0].place}`);
        res.set({
        ...
        res.status(500).send('<svg xmlns="http://www.w3.org/2000/svg" width="117" height="20"><text x="10" y="15" fill="red">Error</text></svg>');
    }
};


// Generate PNG badge
exports.getBadgePng = async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await pool.query(`
            SELECT id, name,
        ...
        const svg = generateBadgeSvgForPng(`#${rows[0].place}`);
        // Convert to PNG without resize - keep original 20px height
...
// Get badge JSON (shields.io compatibility)
exports.getBadge = async (req, res) => {
    try {
        ...
        if (rows.length === 0) {
            return res.status(404).json({ schemaVersion: 1, label: 'OpenYellow', message: 'not found', color: 'red' });
        }

        res.set({ 'Content-Type': 'application/json', 'Cache-Control': 'public, max-age=300' });
        res.json({ schemaVersion: 1, label: 'OpenYellow', message: `#${rows[0].place}`, color: 'yellow' });
    } catch (error) {
        console.error('Error generating badge:', error);
        res.status(500).json({ schemaVersion: 1, label: 'OpenYellow', message: 'error', color: 'red' });
    }
};
