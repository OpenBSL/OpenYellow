...
const dejaVuFontPath = path.join(
...
function generateBadgeSvgForPng(place) {
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="20"><defs><linearGradient id="s" x2="0" y2="100%"><stop offset="0" stop-color="#bbb" stop-opacity=".1"/><stop offset="1" stop-opacity=".1"/></linearGradient><clipPath id="r"><rect width="${totalWidth}" height="20" rx="3" fill="#fff"/></clipPath></defs><g clip-path="url(#r)"><rect width="${labelWidth}" height="20" fill="#555"/><rect x="${labelWidth}" width="${messageWidth}" height="20" fill="#dfb317"/><rect width="${totalWidth}" height="20" fill="url(#s)"/></g>${logoPaths}<g fill="#fff" text-anchor="middle" font-family="DejaVu Sans,sans-serif" font-size="11"><text x="55" y="15" fill="#010101" fill-opacity=".3">OpenYellow</text><text x="55" y="14" fill="#fff">OpenYellow</text><text x="${labelWidth + messageWidth / 2}" y="15" fill="#010101" fill-opacity=".3">${place}</text><text x="${labelWidth + messageWidth / 2}" y="14" fill="#fff">${place}</text></g></svg>`;
}
